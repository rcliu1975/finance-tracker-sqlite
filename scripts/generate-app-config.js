...
function parseArgs(argv) {
  const options = {
    ...
    output: path.join(cwd, "app-config.js")
  };
...
function readConfig(options) {
  ...
  throw new Error(`找不到 ${path.basename(options.envFile)}，也沒有既有 ${path.basename(options.output)} 可沿用。`);
}

...
可依你的實際使用方式分成三組。只跑 SQLite 時，至少備份「應用本體」；若還要保留 Firebase 相容或對外服務能力，再加上對應組別。

#### 應用本體

| 路徑 | 必要性 | 說明 |
| --- | --- | --- |
| repo Git 內容 | 必要 | 原始碼、腳本與 tracked 設定 |
| `.env` | 必要 | 產生 `app-config.js` 的來源；未被 Git 備份 |
| `app-config.js` | 建議 | 前端主線實際載入的 runtime 設定；可由 `.env` 重建 |
| `firebase-config.js` | 視情況 | 舊檔名相容 fallback；只有還要保留舊流程時才需要 |
| `~/finance-tracker.db` | 必要 | 正式 SQLite database |
| `~/finance-tracker-backups/` | 建議 | 平常匯出的 CSV / JSON 備份 |

#### Firebase 相容 / 部署

| 路徑 | 必要性 | 說明 |
| --- | --- | --- |
| `.firebaserc` | 需要 Firebase 部署時必要 | Firebase project id |
| `firebase.json` | 需要 Firebase 部署時必要 | Hosting / rules 部署設定 |
| `firestore.rules` | 需要 Firebase rules 部署時必要 | Firestore rules |
| `firebase-config.example.js` | 可選 | 只作範例參考，不是執行時必需 |
...
`app-config.js` 與 `firebase-config.js` 都可以重建，但 SQLite database、`.env`、cloudflared credentials、`Caddyfile` 遺失後通常最難補回。

### 8. 災難復原 / 還原順序

建議按下面順序還原，避免前端已開起來但 bridge、tunnel 或反向代理還沒接好：

1. 還原 repo 內容到目標機器。
2. 還原 `.env`。
3. 還原 `~/finance-tracker.db` 到原本或新的固定路徑。
4. 若有保留匯出備份，還原 `~/finance-tracker-backups/`。
5. 還原 `~/.config/finance-tracker-sqlite/systemd.env`。
6. 還原 `~/.cloudflared/config.yml` 與 `~/.cloudflared/<tunnel-id>.json`。
7. 還原 `/etc/caddy/Caddyfile`。
8. 若需要 Firebase 部署能力，確認 `.firebaserc`、`firebase.json`、`firestore.rules` 都在。
9. 執行 `npm run config:generate` 重建 `app-config.js`；若仍需舊相容檔，再執行 `node scripts/generate-firebase-config.js`。
10. 若有 user-level systemd service，還原 `~/.config/systemd/user/*.service` 後執行 `systemctl --user daemon-reload`。
11. 重新啟動相關服務並驗證：
...
* **停止與解除安裝服務**：
  ...
  systemctl --user daemon-reload
  ```
# Finance Tracker SQLite Backup / Restore Plan

這份文件假設 `finance-tracker-sqlite` 是從 GitHub `git clone` 取得，Borg 只保存「資料」與「必要的本機設定」。

目標：

- 產出可完整復原的備份
- 使用單一 Borg archive，命名為 `finance-tracker-<date>`
- 提供一個在全新電腦上可手動啟用的還原流程
...
- `~/finance-tracker.db`
- `~/finance-tracker-backups/latest/`

### 1.2 不納入 Borg

- `~/WorkSpace/finance-tracker-sqlite/.env`
- `~/WorkSpace/finance-tracker-sqlite/app-config.js`
...
- `app-config.js` 可以在還原時重新生成
- `cloudflared` 與 Caddy 可以在還原時重新安裝與重新設定
- `.env` 只是在某些情境下方便產生 `app-config.js`，不是 SQLite 還原的必要資料
- `systemd.env` 也可以在還原時重新建立，不需要放進 Borg
...
```bash
borg init --encryption=none user@qnap-host:/share/Backup3/BorgRepo_finance-tracker --remote-path /opt/bin/borg
```
...
```text
finance-tracker-<date>
```

例如：

```text
finance-tracker-2026-07-04
```
...
### 步驟 3: 復製 `finance-tracker.db` 和 `systemd.env`
...
```bash
cd "$HOME"
borg create --stats --progress user@qnap-ip:/share/Backup3/BorgRepo_finance-tracker::finance-tracker-$(date +%F) finance-tracker-backups/latest --remote-path /opt/bin/borg
```
...
復製 ssh 密鑰 `id_ed25519`, `id_ed25519.pub` 到 ~/.ssh
...
```bash
borg list user@qnap-ip:/share/Backup3/BorgRepo_finance-tracker --remote-path /opt/bin/borg
```
...
borg extract user@qnap-ip:/share/Backup3/BorgRepo_finance-tracker::finance-tracker-$DATECODE --remote-path /opt/bin/borg --strip-components 2
```
...
APP_SQLITE_API_BASE_URL=https://moneybook.kennylab.online/bridge
EOF
...
固定只走 SQLite，`app-config.js` 直接重新生成即可，不需要從 Borg 還原。
...
### 步驟 6: 手動建立 frontend systemd service

```bash
mkdir -p "$HOME/.config/systemd/user"
cat > "$HOME/.config/systemd/user/finance-tracker-sqlite-frontend.service" <<EOF
[Unit]
Description=Finance Tracker SQLite frontend
After=network-online.target
Wants=network-online.target
...
[Service]
Type=simple
WorkingDirectory=$HOME/WorkSpace/finance-tracker-sqlite
EnvironmentFile=$HOME/.config/finance-tracker-sqlite/systemd.env
ExecStart=/usr/bin/env bash -lc 'exec "$NPM_BIN" run sqlite:frontend -- --db "$DB_PATH" --user-id "$USER_ID" --bridge-host "$BRIDGE_HOST" --serve-host "$SERVE_HOST" --public-origin "$PUBLIC_ORIGIN" --login-email-env LOGIN_EMAIL --login-password-env LOGIN_PASSWORD'
Restart=always
RestartSec=3
...
```
 ...
 - scripts/run-sqlite-frontend.py 的預設值      
      - --bridge-port 預設 8765      
      - --serve-port 預設 5173      
      - --open-host 預設 127.0.0.1      
      - 這裡是真正決定前端與 bridge 監聽 port 的地方      
  - /etc/caddy/Caddyfile                                                        
      - 決定 8000 上的 / 與 /bridge/* 要轉去q哪裡                            
      - 範例是：                                                                
          - / -> 127.0.0.1:5173                                                 
          - /bridge/* -> 127.0.0.1:8765        
  - ~/.cloudflared/config.yml                                                   
      - 決定外部 hostname 進來後要轉到哪個本機 service                      
      - 把 moneybook.kennylab.online 轉到 http://localhost:8000                     

  - 也就是 Cloudflared 只負責把外網送到 8000          
  - 8000 再由 Caddy 代理到 5173 / 8765                                     
...
可以用這個指令直接確認：
  ...
  ss -ltnp | rg ':5173|:8765'

  我剛剛在這台機器上查過，5173 和 8765 目前都沒有在 LISTEN，也就是沒有程序正在開
  著這兩個 port。

  如果你想更明確驗證服務能不能連上，可以再測：

  curl -I http://127.0.0.1:5173
  curl -I http://127.0.0.1:8765

  如果有服務在跑，通常會回 HTTP header；如果沒開，會連不上。

有，~/.cloudflared/config.yml 之前是錯的；cloudflared service 本身沒問題。

  問題點：

  - /home/roger/.cloudflared/config.yml:4-7 第 6 行原本把 moneybook.kennylab.online 導到 127.0.0.1:8000
  - 但你現在真正的入口是 Caddy 的 127.0.0.1:8080
  - /home/roger/.config/systemd/user/cloudflared-tunnel.service:6-9 只是正常地把 cloudflared 拉起來，沒看到 unit 本身寫錯

  我已經把 config 改成：

  - /home/roger/.cloudflared/config.yml:4-7 -> service: http://127.0.0.1:8080

  而且已重啟確認：

  - cloudflared-tunnel.service 現在是 active (running)
  - 之前日誌裡反覆出現的 dial tcp 127.0.0.1:8000: connect: connection refused 已對應到舊設定
  - 本機 http://127.0.0.1:8080/ 已可正常回應

  補充：

  - 你本機沒有 finance-tracker-sqlite-cloudflared.service，實際存在的是 cloudflared-tunnel.service
  - 如果外網還有問題，下一步就該查 Cloudflare DNS / tunnel route 是否真的指到這條 tunnel，而不是再看 Caddy

  如果你要，我可以接著幫你檢查 cloudflared tunnel route dns 和這個 tunnel 在 Cloudflare 後台對應的 hostname。
